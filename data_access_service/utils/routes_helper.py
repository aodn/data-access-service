...
def _generate_partial_json_array(
    ...
    for partition in ddf.to_delayed():
        ...
        for record in lazy_to_records_fast(partition_df):
            ...
            # Longitude/latitude use the ordered candidates from config.yaml
            # so the first matching key wins according to the defined priority.
            coord_mapping = Config.get_config().get_column_name_mapping()
            for candidate in coord_mapping.get("longitude") or []:
                if candidate in record:
                    val = record[candidate]
                    filtered_record[STR_LONGITUDE_LOWER_CASE] = (
                        round(val, COORDINATE_INDEX_PRECISION)
                        if val is not None
                        else None
                    )
                    break

            for candidate in coord_mapping.get("latitude") or []:
                if candidate in record:
                    val = record[candidate]
