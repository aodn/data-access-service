...
def test_valid_datetime_with_timezone():
    result = _verify_datatime_param("start_date", "2023-01-01T12:00:00+00:00")
    assert isinstance(result, pd.Timestamp)
    assert result.tz is not None


def test_valid_datetime_without_timezone():
    result = _verify_datatime_param("start_date", "2023-01-01T12:00:00")
    assert isinstance(result, pd.Timestamp)
...
def test_non_end_date_does_not_require_nanosecond_precision():
    ...
    result = _verify_datatime_param("start_date", "2023-01-01")
    assert isinstance(result, pd.Timestamp)
