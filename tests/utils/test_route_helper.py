...
from data_access_service.utils.routes_helper import round_coordinate_list, generate_feature_collection
...
def test_round_coordinate_list():
    ...
    expected_coords = [ValueCount(value=1.2, count=1), ValueCount(value=2.3, count=1), ValueCount(value=3.5, count=1)]
    ...
    expected_coords2 = [ValueCount(value=1.2, count=2), ValueCount(value=2.3, count=1), ValueCount(value=3.5, count=1)]
    assert rounded_coords2 == expected_coords2

def test_generate_feature_collection():
    zarr_path = Path(__file__).parent.parent / "canned/s3_sample3/satellite_ghrsst_l4_gamssa_1day_multi_sensor_world.zarr"
    ...
    expected_result_path = Path(__file__).parent.parent / "canned/expected_json/generate_feature_collection_expected.json"
