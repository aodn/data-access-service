...
def sync_aws_batch_job_definition(config: Config, aws: AWSClient, log: Logger):
    ...
    if cloud_job_definition is None:
        log.warning(
            "No cloud job definition found. Registering new job definition: "
            + job_definition_name
        )
        log.warning(
            "New job definition won't be registered for avoliding wrong ARN reference etc.. Please register it manually."
        )
        return

    if is_json_different(
        json_1=cloud_job_definition,
        json_2=local_job_definition,
        ignored_files=batch_json_settings_ignored_field,
    ):
        log.info(
            "Cloud job definition is different from local. Registering new job definition: "
            + job_definition_name
        )
    ...
    log.info(
        "Cloud job definition is the same as local. No need to register a new job definition: "
        + job_definition_name
    )
...
def sync_aws_batch_job_queue(config: Config, aws: AWSClient, log: Logger):
    ...
    if cloud_job_queue is None:
        log.warning(
            "No cloud job queue found. Registering new job queue: " + job_queue_name
        )
        log.warning(
            "New job queue won't be registered for avoliding wrong ARN reference etc.. Please register it manually."
        )
        return
    ...
    if does_job_queue_need_update(
        ...
        log.info(
            "Cloud job queue is different from local. Updating job queue: "
            + job_queue_name
        )
    ...
    log.info(
        "Cloud job queue is the same as local. No need to register a new job queue: "
        + job_queue_name
    )
...
def sync_aws_batch_compute_environment(config: Config, aws: AWSClient, log: Logger):
    ...
    cloud_compute_environment = aws.get_batch_compute_environment_config(
    ...
    if cloud_compute_environment is None:
        log.warning(
            "No cloud compute environment found. Registering new compute environment: "
            + compute_environment_name
        )
        log.warning(
    ...
    if does_compute_environment_need_update(
        ...
        log.info(
            "Cloud compute environment is different from local. Updating compute environment: "
            + compute_environment_name
        )
    ...
    log.info(
        "Cloud compute environment is the same as local. No need to register a new compute environment: "
        + compute_environment_name
    )
...
def get_local_json(config: Config, path: Path) -> dict:
    ...
    return replace_json_values(
        json_data=local_job_definition,
        replacements=config.get_job_definition_distinct_fields(),
    )


def does_job_queue_need_update(
    cloud_job_queue: dict, local_job_queue: dict, log: Logger
) -> bool:
...
def does_compute_environment_need_update(
    cloud_compute_environment: dict, local_compute_environment: dict, log: Logger
) -> bool:
    ...
    for key, value in local_compute_environment.items():
        ...
        if key == "computeEnvironment":
            ...
            if is_json_different(
                value,
                cloud_compute_environment.get(key, {}),
                ignored_files=["root['type']"],
            ):
                log.info(f"Compute environment needs update")
                return True
