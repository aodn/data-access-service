...
class TestParquetProcessorWithS3(TestWithS3):
    ...
    def test_parquet_processor_seagrass_with_s3(
        self,
        aws_clients,
        setup_resources,
        ...
        SEAGRASS_KEY: Final = "aggregated_seagrass_nonqc.parquet"
        SEAGRASS_UUID: Final = "009a1131-efc1-4a61-8f90-cf289e7c043d"
        MASTER_JOB_ID: Final = "542e76c3-6c19-463e-8b85-0b62db45f045"
        ...
        SEAGRASS_PREPARATION_PARAMETERS: Final = {
    ...
    @patch("aodn_cloud_optimised.lib.DataQuery.REGION", REGION)
    def test_parquet_processor_animal_metadata_with_s3(
        self,
        aws_clients,
        setup_resources,
        upload_test_case_to_s3,
        ...
        ANIMAL_METADATA_KEY: Final = (
            "animal_metadata_satellite_relay_tagging_location_qc_realtime_qc.parquet"
        )
        ANIMAL_METADATA_UUID: Final = "b2548767-514f-4a31-b65e-36bb894382d5"
        MASTER_JOB_ID: Final = "8905-animal-metadata-subset"
        # Hive partition timestamp=1764547200 is 2025-12-01 UTC.
        ANIMAL_METADATA_JOB_INDEX = "0"

        aodn = DataQuery.GetAodn()
        metadata: Metadata = aodn.get_metadata()
        assert metadata.metadata_catalog().get(ANIMAL_METADATA_KEY) is not None

        api = API()
        api.initialize_metadata()

        ANIMAL_METADATA_PREPARATION_PARAMETERS: Final = {
            "end_date": "non-specified",
            "date_ranges": json.dumps(
                {
                    "0": [
            ...
            "master_job_id": MASTER_JOB_ID,
            "full_metadata_link": (
                "https://portal.aodn.org.au/details/" f"{ANIMAL_METADATA_UUID}"
            ),
            "type": "sub-setting-data-preparation",
            "uuid": ANIMAL_METADATA_UUID,
            "intermediate_output_folder": f"/tmp/tmpanimalmeta{MASTER_JOB_ID}",
            "collection_title": (
                "Animal satellite relay tagging metadata (location QC, realtime)"
            ),
            "suggested_citation": "",
            "output_format": "csv",
        ...
        with patch.object(AWSHelper, "send_email"):
            try:
                prepare_data(
                    api,
                    job_index=ANIMAL_METADATA_JOB_INDEX,
                    parameters=ANIMAL_METADATA_PREPARATION_PARAMETERS,
                )
                ...
                assert f"{MASTER_JOB_ID}/temp/dataschema.json" in objects, (
                    f"prepare_data did not upload a schema for {ANIMAL_METADATA_KEY}: "
                    f"{objects}"
                )
                parquet_objects = [key for key in objects if key.endswith(".parquet")]
                assert parquet_objects, (
                    f"prepare_data did not upload subset parquet for "
                    f"{ANIMAL_METADATA_KEY}: {objects}"
                )
                ...
                date_ranges = json.loads(
                    ANIMAL_METADATA_PREPARATION_PARAMETERS["date_ranges"]
                )
                start_date = parse_date(date_ranges[ANIMAL_METADATA_JOB_INDEX][0])
                end_date = parse_date(date_ranges[ANIMAL_METADATA_JOB_INDEX][1])
                ...
                shutil.rmtree(config.get_temp_folder(MASTER_JOB_ID), ignore_errors=True)
                shutil.rmtree(
                    ANIMAL_METADATA_PREPARATION_PARAMETERS[
