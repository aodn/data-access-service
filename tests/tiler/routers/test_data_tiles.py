...
def test_list_products_includes_coastal_fill_only_when_present(tmp_path, monkeypatch):
    ...
    r = client.get("/tiler/data_tiles/products")
...
def test_tile_unknown_product():
    response = client.get("/tiler/data_tiles/nonexistent/2024-01-01/1/0/0.png")
...
def test_manifest_unknown_product():
    response = client.get("/tiler/data_tiles/nonexistent/2024-01-01/manifest.json")
    assert response.status_code == 404
...
def test_inspect_unknown_product():
    response = client.get("/tiler/data_tiles/nonexistent/inspect")
    assert response.status_code == 404
