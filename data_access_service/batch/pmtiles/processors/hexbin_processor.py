...
class HexbinProcessor(AbstractProcessor):
    ...
    def build_staging_parquet(self):
        ...
        quoted_lon = quote_identifier(lon_name)
        quoted_lat = quote_identifier(lat_name)
        quoted_time = quote_identifier(time_col_name)
        time_type = detect_time_type(
            self.con, input_path=self.get_s3_uri(), time_col=time_col_name
        )
        ym = build_ym_expression(time_col=time_col_name, time_type=time_type)
