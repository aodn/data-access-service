...
def _write_variable_parquet(tiler_root_dir, store, variable, ts, rows):
...
def test_fetch_variable_slice_fills_only_the_rows_present(tiler_root_dir, session):
    _write_variable_parquet(tiler_root_dir, "x", "v", TS, [(0, 0, 1.5)])
...
def test_fetch_variable_slice_reads_the_file_for_that_timestamp(
    tiler_root_dir, session
):
    other = "2024-01-15T14:00:00.000000000Z"
    _write_variable_parquet(tiler_root_dir, "x", "v", TS, [(0, 0, 1.0)])
    _write_variable_parquet(tiler_root_dir, "x", "v", other, [(0, 0, 2.0)])
    repo = TilerParquetRepository(session)
...
def test_fetch_variable_slice_resolves_relative_to_the_tiler_root_dir(
    tiler_root_dir, session
):
    _write_variable_parquet(tiler_root_dir, "foo", "v", TS, [(0, 0, 9.0)])
    repo = TilerParquetRepository(session)
...
def test_fetch_variable_slice_of_an_empty_file_is_all_nan(tiler_root_dir, session):
    """A variable with no valid cells at an instant another variable of the
    same store does cover."""
    _write_variable_parquet(tiler_root_dir, "x", "v", TS, [])
    repo = TilerParquetRepository(session)
...
def test_fetch_variable_slice_uses_int16_indexes_when_they_fit(tiler_root_dir, session):
    _write_variable_parquet(tiler_root_dir, "x", "v", TS, [(1, 2, 3.0), (0, 1, 4.0)])
    repo = TilerParquetRepository(session)
...
def test_fetch_variable_slice_keeps_int32_indexes_for_a_wide_grid(
    tiler_root_dir, session
):
    _write_variable_parquet(tiler_root_dir, "x", "v", TS, [(0, 40000, 1.0)])
    repo = TilerParquetRepository(session)
...
def test_fetch_variable_slice_raises_when_the_file_is_missing(tiler_root_dir, session):
    repo = TilerParquetRepository(session)
